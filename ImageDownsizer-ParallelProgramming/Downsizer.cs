        ...
        private void NearestNeighborTypeInterpolation(BitmapData originalData, int newWidth, int newHeight, BitmapData newImageData, int pixelSize, byte[] originalBytes, byte[] downsizedImageBytes, double ratio, int startY = 0, int endY = -1)
        {
            ...
            for (int y = startY; y < endY; y++)
            {
                for (int x = 0; x < newWidth; x++)
                {
                    ...
                    byte[] averageColor = WeightedAverageColor(originalBytes, originalData.Stride, x1, y1, x2, y2, ratio, pixelSize);

                    int index = y * newImageData.Stride + x * pixelSize;

                    downsizedImageBytes[index] = averageColor[0];
                    downsizedImageBytes[index + 1] = averageColor[1];
                    downsizedImageBytes[index + 2] = averageColor[2];

                    if (pixelSize == 4)
                    {
                        downsizedImageBytes[index + 3] = averageColor[3];
                    }
